...
export class EChartsTransitionRenderer extends EChartRenderer {
  constructor(host) {
    super(host);
    this.outcome = "crossed";
    this.display = "fraction";
    this.minimumSupport = 5;
    this.inspectHandler = null;
    this.chart.on("click", params => {
      if (params.seriesType === "heatmap" && params.data && this.inspectHandler) this.inspectHandler(params.data);
    });
  }
  setInspectHandler(handler) { this.inspectHandler = handler; }
  setVisualOptions({outcome, display, minimumSupport} = {}) {
    this.outcome = outcome === "ended" ? "ended" : "crossed";
    this.display = display === "count" ? "count" : "fraction";
    this.minimumSupport = Math.max(1, Math.floor(Number(minimumSupport) || 1));
    this.draw();
  }
  option() {
    const data = this.data;
    const count = Math.max(1, data.panelCount), cols = autoColumns(count, data.columns), rows = Math.ceil(count / cols);
    const width = Math.max(320, this.host.clientWidth), height = Math.max(360, this.host.clientHeight);
    const top = 58, bottom = 42, cellW = width / cols, cellH = (height - top - bottom) / rows;
    const grid = [], xAxis = [], yAxis = [], titles = [], series = [];
    const valuesByPanel = [];
    let maximum = this.display === "fraction" ? 1 : 1;
    const success = this.outcome === "ended" ? data.ended : data.crossed;
    for (let panel = 0; panel < count; panel += 1) {
      const col = panel % cols, row = Math.floor(panel / cols);
      const square = Math.max(60, Math.min(cellW - 70, cellH - 48));
      grid.push({left: col * cellW + (cellW - square) / 2, top: top + row * cellH + 24, width: square, height: square});
      const xValues = Array.from({length: data.nx}, (_, index) => data.x0 + (index + .5) * data.bin);
      const zValues = Array.from({length: data.nz}, (_, index) => data.z0 + (index + .5) * data.bin);
      const xEvery = Math.max(0, Math.ceil(data.nx / 6) - 1), zEvery = Math.max(0, Math.ceil(data.nz / 6) - 1);
      xAxis.push({...axisBase(), gridIndex: panel, type: "category", data: xValues, boundaryGap: true, axisLabel: {...axisBase().axisLabel, interval: xEvery, formatter: value => formatNumber(Number(value), 1)}});
      yAxis.push({...axisBase(), gridIndex: panel, type: "category", data: zValues, boundaryGap: true, axisLabel: {...axisBase().axisLabel, interval: zEvery, formatter: value => formatNumber(Number(value), 1)}});
      titles.push({text: data.panelNames?.[panel] || "All data", left: col * cellW + (cellW - square) / 2, top: top + row * cellH, textStyle: {fontSize: 11, fontWeight: 650, color: INK}});
      const panelValues = [];
      const cells = data.nx * data.nz;
      for (let iz = 0; iz < data.nz; iz += 1) for (let ix = 0; ix < data.nx; ix += 1) {
        const index = panel * cells + iz * data.nx + ix;
        const support = data.entered[index];
        if (support < this.minimumSupport) continue;
        const value = this.display === "fraction" ? success[index] / support : success[index];
        maximum = Math.max(maximum, value);
        panelValues.push({
          value: [ix, iz, value], support, successes: success[index],
          x: xValues[ix], z: zValues[iz], panelIndex: panel, panel: data.panelNames?.[panel] || "All data",
        });
      }
      valuesByPanel.push(panelValues);
      series.push({
        name: data.panelNames?.[panel] || "All data", type: "heatmap",
        xAxisIndex: panel, yAxisIndex: panel, data: panelValues,
        progressive: 3000, itemStyle: {borderWidth: 0},
        emphasis: {itemStyle: {borderColor: "#17211f", borderWidth: 1}},
      });
    }
    const axisIndices = Array.from({length: count}, (_, index) => index);
    return {
      ...this.base("transition-probability"), title: titles,
      tooltip: {trigger: "item", confine: true, formatter: params => {
        const item = params.data || {};
        return `<b>${escapeHtml(item.panel)}</b><br>Cell ${formatNumber(item.x, 1)}, ${formatNumber(item.z, 1)}<br>${escapeHtml(this.outcome)}: <b>${this.display === "fraction" ? `${formatNumber(item.value?.[2] * 100, 1)}%` : formatNumber(item.value?.[2], 0)}</b><br>${formatNumber(item.successes, 0)} of ${formatNumber(item.support, 0)} entered segments`;
      }},
      visualMap: {
        min: 0, max: this.display === "fraction" ? 1 : maximum, calculable: true,
        orient: "horizontal", left: "center", top: 7, itemWidth: 11, itemHeight: 120,
        text: [this.display === "fraction" ? "100%" : formatNumber(maximum, 0), "0"],
        textStyle: {color: MUTED, fontSize: 9},
        inRange: {color: ["#440154", "#3b528b", "#21918c", "#5ec962", "#fde725"]},
      },
      dataZoom: [{type: "inside", xAxisIndex: axisIndices, yAxisIndex: axisIndices, filterMode: "none", throttle: 35}],
      grid, xAxis, yAxis, series,
      graphic: [{type: "text", left: 12, top: 29, silent: true, style: {
        text: `split Z ${formatNumber(data.split, 1)} · support ≥ ${this.minimumSupport} · click a cell for paths`,
        fill: MUTED, font: "9px Inter, system-ui",
      }}],
    };
  }
}
